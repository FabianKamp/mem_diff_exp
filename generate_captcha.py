...
for i in range(5): 
    text = ''.join(random.choices(list("memrdiffusin"), k=3)) + ''.join(random.choices(string.digits, k=2))
    ...
    for _ in range(4):
        ...
        draw.line((x1, y1, x2, y2), fill=(150, 150, 150), width=random.randint(1,3))
    ...
    # save
    img.save(f'./captcha/captcha{i+1}.jpg', 'JPEG', quality=60, optimize=True)